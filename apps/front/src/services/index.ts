...
const configCom = computed(() => {
    return {
        baseURL: import.meta.env.VITE_BACKENDCOM_URL,
        headers: {
            "Content-Type": "application/json",
        }
    }
...
const clientCom: AxiosInstance = axios.create(configCom.value);
...
clientCom.interceptors.request.use(
    function (config: any) {
        config.headers.Authorization = `Bearer ${token.value ? token.value : ''}`;
        return config;
    },
...
export { client, clientWithoutAuth, clientFormData, clientCom };